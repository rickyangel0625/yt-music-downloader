...
if ($latestFile) {
    # 使用者選擇是否放大音量至 300%
    $amplify = Read-Host "Do you want to increase 300% volume?(y/n)"
    # 使用 ffmpeg 轉換為 mp3 格式
    $outputFile = "$desktop\$($latestFile.BaseName).mp3"
    # 建立 ffmpeg 參數清單
    if ($amplify -eq "y") {
        ffmpeg -i "`"$($latestFile.FullName)`"" -filter:a "volume=3.0" -vn -ar 44100 -ac 2 -b:a 128k "`"$outputFile`""
        Write-Host "Converted and amplified $outputFile"
    }
    else {
        ffmpeg -i "`"$($latestFile.FullName)`"" -vn -ar 44100 -ac 2 -b:a 128k "`"$outputFile`""
        Write-Host "Convert to original volume $outputFile"
    }
